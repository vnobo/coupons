...
(
    64
) NOT NULL UNIQUE COMMENT '账户微信ID',
    ` nickname ` varchar
(
    64
) COMMENT '微信昵称',
...
(
    20
) UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT '事件ID',
    ` event ` varchar
(
    64
...
(
    1
) NOT NULL DEFAULT 1 COMMENT '订单类型1,淘宝.2,京东.3,拼多多',
    ` num_iid ` VARCHAR
(
    64
